...
const PRODUCT_TO_PLAN: Record<string, ProductPlanMapping> = {
...
async function validateAppleReceipt(receipt: string, sharedSecret: string): Promise<{
    valid: boolean;
    expiresAt?: string;
    productId?: string;
}> {
    ...
    for (const url of urls) {
        try {
            ...
            if (!latestInfo) return { valid: false };
...
export const validateReceipt = onCall(async (request) => {
    ...
    const { receipt, productId, platform } = request.data as ValidateReceiptRequest;

    // 2. Input validasyonu
    if (!receipt || !productId || !platform) {
        throw new HttpsError('invalid-argument', 'Eksik alanlar: receipt, productId, platform');
    }

    if (!['ios', 'android'].includes(platform)) {
        throw new HttpsError('invalid-argument', 'Geçersiz platform. ios veya android olmalı.');
    }

    const planMapping = PRODUCT_TO_PLAN[productId];
    if (!planMapping) {
        throw new HttpsError('invalid-argument', `Bilinmeyen ürün ID: ${productId}`);
    }
    ...
    if (!companyId) {
        throw new HttpsError('failed-precondition', 'Kullanıcının bir firmaya ait olması gerekiyor.');
    }

    // 4. Sadece admin/owner plan değiştirebilir
    if (!['admin', 'superadmin'].includes(userRole)) {
        throw new HttpsError('permission-denied', 'Plan değiştirme yetkiniz yok. Sadece firma yöneticileri plan değiştirebilir.');
    }

    // 5. Firma varlık kontrolü
    const companyDoc = await db.collection('companies').doc(companyId).get();
    if (!companyDoc.exists) {
        throw new HttpsError('not-found', 'Firma bulunamadı.');
    }
    ...
    try {
        ...
        if (!validationResult.valid) {
            throw new HttpsError('invalid-argument', 'Receipt doğrulama başarısız.');
        }
        ...
        throw new HttpsError('internal', 'Sunucu hatası oluştu.');
    }
});
...
export const sendNotification = onCall(async (request) => {
    ...
    const uid = request.auth.uid;
    const { targetTokens, title, body, data } = request.data as {
        targetTokens: string[];
        title: string;
        body: string;
    ...
    if (!targetTokens || !title || !body) {
        throw new HttpsError('invalid-argument', 'Eksik alanlar: targetTokens, title, body');
    }
 ...
 */
export const checkExpiredSubscriptions = onSchedule('every 24 hours', async () => {
    try {
 ...
 */
function calculateDocSize(data: any): number {
    if (!data) return 0;
    let size = 0;
